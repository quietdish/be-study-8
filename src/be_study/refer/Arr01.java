...
public class Arr01 {
	...
	public static void main(String[] args) {
		...
		//index: 0 1 2 3 4
		sum = 0;  // i<5;
		for(int i=0; i<=4; i++) {  // i: 0 1 2 3 4 5	| 5
			sum = sum + scoreArr[i];
		}
		avg = sum / 5.0;
		System.out.println("총점: " + sum + " 평균: " + avg);
		...
		// pointArr = 20;	// pointArr : 5개 공간의 주소를 참조하고 있는 참조타입변수 !! -> 불가능 
		...
		int[] intArr = new int[6];	//for(int i=0; i<6; i++) { ?6바뀌면같이바꿔야되는데 아래처럼..
		...
		//for(int i=0; i<6; i++) {
		//	int i=0; i<6; i++
		for(int i=0; i< intArr.length; i++) {				//?요기~!
		...
		System.out.println();
		for(int i=0; i<stringArr.length; i++) {	  //최대인덱스 : 배열길이 - 1
			System.out.print(stringArr[i] + " "); //배열길이 30 인덱스 0~29
		}
		System.out.println();
